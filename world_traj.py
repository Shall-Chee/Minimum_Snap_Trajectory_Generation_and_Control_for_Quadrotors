...
class WorldTraj(object):
    ...
    def __init__(self, world, start, goal):
        ...
        for i in range((self.points_list).shape[0] - 1):
            self.time_nodes.append(self.time_nodes[-1] + 2 * np.sqrt(np.linalg.norm(
                self.points_list[i + 1] - self.points_list[
                    i]) / self.acc_value))  # get time_nodes, each time section is 2*length/acceleration
            self.direction = np.vstack((self.direction,
                                        (self.points_list[i + 1] - self.points_list[i]) / np.linalg.norm(
                                            self.points_list[i + 1] - self.points_list[i])))  # get direction, norm is 1
    ...
    def update(self, t):
        ...
        for i in range(3):
            ...
            for j in range(n - 1):  # q
                # print(j)
                cons = cons + 1
                A[cons, 6 * j:6 * (j + 1), i] = [self.time_nodes[j + 1] ** 5, self.time_nodes[j + 1] ** 4,
                                                 self.time_nodes[j + 1] ** 3, self.time_nodes[j + 1] ** 2,
                                                 self.time_nodes[j + 1], 1]
                b[cons, i] = self.points_list[j + 1, i]
                cons = cons + 1
                A[cons, 6 * (j + 1):6 * (j + 2), i] = [self.time_nodes[j + 1] ** 5, self.time_nodes[j + 1] ** 4,
                                                       self.time_nodes[j + 1] ** 3, self.time_nodes[j + 1] ** 2,
                                                       self.time_nodes[j + 1], 1]
                b[cons, i] = self.points_list[j + 1, i]
            ...
            for j in range(n - 1):  # velocity continue
                cons = cons + 1
                A[cons, 6 * j:6 * (j + 1), i] = [5 * self.time_nodes[j + 1] ** 4, 4 * self.time_nodes[j + 1] ** 3,
                                                 3 * self.time_nodes[j + 1] ** 2, 2 * self.time_nodes[j + 1], 1, 0]
                ...
                A[cons, 6 * (j + 1):6 * (j + 2), i] = [-5 * self.time_nodes[j + 1] ** 4,
                                                       -4 * self.time_nodes[j + 1] ** 3,
                                                       -3 * self.time_nodes[j + 1] ** 2, -2 * self.time_nodes[j + 1],
                                                       -1, 0]
                b[cons, i] = 0
            for j in range(n - 1):  # acc continue
                cons = cons + 1
                A[cons, 6 * j:6 * (j + 1), i] = [20 * self.time_nodes[j + 1] ** 3, 12 * self.time_nodes[j + 1] ** 2,
                                                 6 * self.time_nodes[j + 1], 2, 0, 0]
                A[cons, 6 * (j + 1):6 * (j + 2), i] = [-20 * self.time_nodes[j + 1] ** 3,
                                                       -12 * self.time_nodes[j + 1] ** 2, -6 * self.time_nodes[j + 1],
                                                       -2, 0, 0]
                b[cons, i] = 0
            for j in range(n - 1):  # jerk continue
                cons = cons + 1
                A[cons, 6 * j:6 * (j + 1), i] = [60 * self.time_nodes[j + 1] ** 2, 24 * self.time_nodes[j + 1], 6, 0, 0,
                                                 0]
                A[cons, 6 * (j + 1):6 * (j + 2), i] = [-60 * self.time_nodes[j + 1] ** 2, -24 * self.time_nodes[j + 1],
                                                       -6, 0, 0, 0]
                b[cons, i] = 0
            for j in range(n - 1):  # snap continue
                ...
                # start
            cons = cons + 1
            A[cons, 0:6, i] = [self.time_nodes[0] ** 5, self.time_nodes[0] ** 4, self.time_nodes[0] ** 3,
                               self.time_nodes[0] ** 2, self.time_nodes[0], 1]
            b[cons, i] = self.points_list[0, i]
            cons = cons + 1
            A[cons, 0:6, i] = [5 * self.time_nodes[0] ** 4, 4 * self.time_nodes[0] ** 3, 3 * self.time_nodes[0] ** 2,
                               2 * self.time_nodes[0], 1, 0]
            b[cons, i] = 0
            cons = cons + 1
            A[cons, 0:6, i] = [20 * self.time_nodes[0] ** 3, 12 * self.time_nodes[0] ** 2, 6 * self.time_nodes[0], 2, 0,
                               0]
            b[cons, i] = 0

            # goal
            cons = cons + 1
            A[cons, 6 * (n - 1):6 * n, i] = [self.time_nodes[-1] ** 5, self.time_nodes[-1] ** 4,
                                             self.time_nodes[-1] ** 3, self.time_nodes[-1] ** 2, self.time_nodes[-1], 1]
            b[cons, i] = self.points_list[n, i]
            cons = cons + 1
            A[cons, 6 * (n - 1):6 * n, i] = [5 * self.time_nodes[-1] ** 4, 4 * self.time_nodes[-1] ** 3,
                                             3 * self.time_nodes[-1] ** 2, 2 * self.time_nodes[-1], 1, 0]
            b[cons, i] = 0
            cons = cons + 1
            A[cons, 6 * (n - 1):6 * n, i] = [20 * self.time_nodes[-1] ** 3, 12 * self.time_nodes[-1] ** 2,
                                             6 * self.time_nodes[-1], 2, 0, 0]
            b[cons, i] = 0
